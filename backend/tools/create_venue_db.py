...
def create_venue_db(df=None, user_features_df=None, output_path=None):
    if df is None:
        ...
        print(f"Lese Check-ins aus {input_path}...")
        try:
    ...
    print(f"Gefundene Check-ins: {len(df)}")
    ...
    if missing_cols:
        print(f"Fehler: Fehlende Spalten in der Datei: {missing_cols}")
        print(f"Vorhandene Spalten: {df.columns.tolist()}")
        return None
        
    print("Extrahiere einzigartige Venues und berechne Popularität...")
    ...
    if user_features_df is not None and 'cluster' in user_features_df.columns:
        print("Berechne Cluster-spezifische Visit-Counts...")
    ...
    print(f"Gefundene einzigartige Venues: {len(venues_df)}")
    
    print(f"Speichere Venues in {output_path}...")
